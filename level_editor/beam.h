...
class Beam {
 ...
 public:
  Beam(float posx, float posy, float angle, float width) : pos_x(posx), pos_y(posy), angle(angle), length(width) {}
  ...
  void render(client_SDL& client_sdl, SDL2pp::Texture* sb_text, SDL2pp::Texture* lb_text) {
    ...
    if (length == 3.0) {
      ...
      client_sdl.renderer.Copy(*sb_text, SDL2pp::Rect(0, 0, sbeam_width - 10, sbeam_height),  // Size
                               SDL2pp::Rect(pos_x_px, pos_y_px, sbeam_width + 20, sbeam_height), -angle,
      ...
      client_sdl.renderer.Copy(*lb_text, SDL2pp::Rect(0, 0, lbeam_width - 10, lbeam_height),  // Size
                               SDL2pp::Rect(pos_x_px, pos_y_px, lbeam_width + 20, lbeam_height), -angle,
                               pivot,  // rotation center - not needed
                               SDL_FLIP_NONE);
    } else {
