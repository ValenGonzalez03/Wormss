...
int Client::run() {
  ...
  resource_pool.add_short_beam();
  resource_pool.add_long_beam();
  // Add resources is not implemented yet
  ...
  // texture
  std::string image_path = "/Images/Worms/waccuse.png";
  ...
  */
  ...
  while (1) {
    ...
    if (is_running) {
      ...
      src_x = 0;
      src_y = 0 + 60 * run_phase;
    }

    // Draw player sprite
    sprites.SetAlphaMod(255); // sprite is fully opaque
    renderer.Copy(  sprites,
                    Rect(src_x, src_y, 60, 60), // Size
                    Rect((int)position, vcenter - 60, 60, 60) // Destination
            );
    
    // Draw one beam
    resource_pool.get_short_beam_texture()->SetAlphaMod(255);
    int sbeam_height = resource_pool.get_short_beam_texture()->GetHeight();
    int sbeam_width = resource_pool.get_short_beam_texture()->GetWidth();
    renderer.Copy(*resource_pool.get_short_beam_texture(),
                    Rect(0, 0, sbeam_width-10, sbeam_height), // Size
                    Rect(0, 0, 75, sbeam_height)); // Destination

    // Draw another beam
    resource_pool.get_long_beam_texture()->SetAlphaMod(255);
    int lbeam_height = resource_pool.get_long_beam_texture()->GetHeight();
    int lbeam_width = resource_pool.get_long_beam_texture()->GetWidth();
    //std::cout << "lbeam_heigth" << lbeam_height << std::endl;
    //std::cout << "lbeam_width" << lbeam_width << std::endl;
    renderer.Copy(*resource_pool.get_long_beam_texture(),
                    Rect(0, 0, lbeam_width-10, lbeam_height), // Size
                    Rect(30, 30, 150, lbeam_height)); // Destination
