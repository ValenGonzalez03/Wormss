...
bool Camera::is_down_corner(int mouse_x, int mouse_y) {
  return mouse_y > camera_box.h * (1 - 1.0f / static_cast<float>(PORTION_SCROLLABLE)) &&
         mouse_y < camera_box.h;
}
...
bool Camera::is_right_corner(int mouse_x, int mouse_y) {
  return mouse_x > camera_box.w * (1 - 1.0f / static_cast<float>(PORTION_SCROLLABLE)) &&
         mouse_x < camera_box.w;
}
