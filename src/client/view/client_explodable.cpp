...
Explodable::Explodable(int pos_x, int pos_y, int width, int heigth, float angle,
                       uint8_t dir, uint8_t id, SDL2pp::Texture* texture,
                       SDL2pp::Renderer& rend) :
    pos_x(pos_x), pos_y(pos_y), width(width), height(heigth), angle(angle),
...
void Explodable::render(int frame, int camera_x, int camera_y) {
  ...
  renderer.Copy(*texture, SDL2pp::NullOpt,
                SDL2pp::Rect(pos_x - camera_x, pos_y - camera_y, width, height),
                -angle_deg, SDL2pp::NullOpt, flip);
...
void Explodable::DrawRotatedRect(SDL2pp::Renderer& renderer, int width, int height,
                                 int pos_x, int pos_y, float angle) {
  int hw = (width * 0.5f);
  int hh = (height * 0.5f);
  ...
  for (int i = 0; i < 4; ++i) {
    int j = (i + 1) % 4;
    renderer.DrawLine(
        static_cast<int>(worldVerts[i].x), static_cast<int>(worldVerts[i].y),
