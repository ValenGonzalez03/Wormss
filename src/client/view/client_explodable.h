...
class Explodable {
 private:
  ...
  void DrawRotatedRect(SDL2pp::Renderer& renderer, int width, int height, int pos_x,  // NOLINT(runtime/references)
 ...
 public:
  ...
  explicit Explodable(int pos_x, int pos_y, int width, int heigth, float angle, uint8_t dir, uint8_t id,
                      SDL2pp::Texture* texture, SDL2pp::Renderer& rend);  // NOLINT(runtime/references)
