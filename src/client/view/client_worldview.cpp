...
void WorldView::add_beam(float pos_x, float pos_y, float width, float height, float angle) {
  ...
  Beam beam(width_px, height_px, pos_x_px, pos_y_px, angle_deg, beam_texture, renderer);
...
void WorldView::update(const GameState &game_state, int frame) {
  ...
  for (auto explodable : explodables) {
    if (explodables_data_aux.find(explodable.first) ==
        explodables_data_aux
            .end()) {  // No encontró el elemento en missiles_data, quiere decir que el misil explotó (o desaparecio (?))
  ...
  for (
      auto data :
      explodables_data_aux) {  // Itero sobre los misiles que quedan de missiles_data, que son los que NO tiene missiles por lo que debo crearlos.
...
void WorldView::set_background(const std::string &path) { resource_pool.add_background(path); }
