...
class StopAiming : public Command {
 public:
  ...
  explicit StopAiming(uint8_t client_id) :
      Command(CODE_PLAYER_COMM::STOP_AIMING, client_id) {}

  // Constructor from server side with direction received by socket
  explicit StopAiming(uint8_t clt_id, Socket &skt,  // NOLINT(runtime/references)
                      bool *was_closed) : Command(CODE_PLAYER_COMM::STOP_AIMING, clt_id) {
