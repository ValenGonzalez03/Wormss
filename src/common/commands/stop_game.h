...
class StopGame : public Command {
 public:
  ...
  explicit StopGame(uint8_t client_id) :
      Command(CODE_PLAYER_COMM::STOP_GAME, client_id) {}

  // Constructor from server side for code consistency but doesn't do anything
  // different from the other
  explicit StopGame(uint8_t clt_id, Socket &skt,  // NOLINT(runtime/references)
                    bool *was_closed) : Command(CODE_PLAYER_COMM::STOP_GAME, clt_id) {
