...
struct WormData {
  ...
  WormData() :
      id(-1), player_id(-1), health(100), pos_x(0), pos_y(0), direction(RIGHT),
      state(IDLE), current_weapon(BAZOOKA), aim_angle(0) {}

  explicit WormData(uint8_t id, uint8_t player_id, uint32_t health, float pos_x,
                    float pos_y, uint8_t dir, WormState st, WeaponType wp, float angle) :
      id(id), player_id(player_id), health(health), pos_x(pos_x), pos_y(pos_y),
      direction(dir), state(st), current_weapon(wp), aim_angle(angle) {}
  ...
  void serialize(ServerProtocol &prot, bool *was_closed) {  // NOLINT(runtime/references)
    // Envio el worm_id
    prot.send_byte(this->id, was_closed);
...
struct GameState {
  ...
  uint8_t current_turn_player_id = -1;
  uint8_t current_turn_worm_id = -1;
  ...
  GameState(ClientProtocol &prot, bool *was_closed) : worms_list() {
    ...
    for (int i = 0; i < worms_amount; i++) {
      WormData worm(prot);
      worms_list.insert(std::pair<uint8_t, WormData>(worm.id, worm));
    }
    for (int i = 0; i < explodables_amount; i++) {
      ExplodableData explodable(prot);
      explodables_list.insert(
          std::pair<uint8_t, ExplodableData>(explodable.id, explodable));
    }
    ...
    this->current_turn_player_id = prot.recv_byte(was_closed);
    this->current_turn_worm_id = prot.recv_byte(was_closed);
  ...
  void serialize(ServerProtocol &prot, bool *was_closed) {  // NOLINT(runtime/references)
    ...
    prot.send_byte(current_turn_player_id, was_closed);
    prot.send_byte(current_turn_worm_id, was_closed);
  ...
  void add_worm(const WormAttr &attr) {
    WormData worm(attr.id, attr.player_id, attr.health, attr.pos_x, attr.pos_y,
                  attr.direction, attr.state, attr.current_weapon, attr.aim_angle);
    worms_list.insert(std::pair<uint8_t, WormData>(worm.id, worm));
  }
