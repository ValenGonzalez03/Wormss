...
Player::Player(uint8_t player_id, Game* game, GamesHandler& games_handler, PlayerSender& sender, std::shared_ptr<Queue<GameState>> sender_queue, ServerProtocol& protocol)
    : player_id(player_id), game(game), games_handler(games_handler), sender(sender), sender_queue(sender_queue), protocol(protocol) {}
