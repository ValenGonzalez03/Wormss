...
Queue<std::unique_ptr<Command>>* GamesHandler::create_game(std::shared_ptr<Queue<GameState*>> sender_queue, const int& player_id) {
	std::lock_guard<std::mutex> lck(m);
...
Queue<std::unique_ptr<Command>>* GamesHandler::join_game(std::shared_ptr<Queue<GameState*>> sender_queue, const int& player_id, const int& game_id) {
	std::lock_guard<std::mutex> lck(m);
	for (auto& current_game: games) {
        if (current_game->compare_id(game_id)) {
